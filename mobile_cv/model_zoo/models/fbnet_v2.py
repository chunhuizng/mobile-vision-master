...
def _create_builder(arch_name_or_def: typing.Union[str, dict], scale_factor=1.0):
...
class FBNetBackbone(nn.Module):
    def __init__(self, arch_name, dim_in=3, scale_factor=1.0):
        super().__init__()

        builder, arch_def = _create_builder(arch_name, scale_factor=scale_factor)
...
class FBNet(nn.Module):
    def __init__(self, arch_name, dim_in=3, num_classes=1000, scale_factor=1.0):
        super().__init__()
        self.backbone = FBNetBackbone(arch_name, dim_in, scale_factor=scale_factor)
...
def fbnet(arch_name, pretrained=False, progress=True, scale_factor=1.0, **kwargs):
    """
    ...
    model = FBNet(arch_name, scale_factor=scale_factor, **kwargs)
    if pretrained:
        assert (
